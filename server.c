...
typedef struct Message {
    ...
    char          filename[MAX_FILENAME]; /* nombre fichero adjunto (Parte 2), "" si ninguno */
...
static pthread_mutex_t users_mutex = PTHREAD_MUTEX_INITIALIZER; /* mutex de la tabla */
...
static unsigned int enqueue_message(int idx, const char *sender,
    ...
    /* Calcular siguiente id: unsigned int, cuando llega a UINT_MAX+1 vuelve a 1 */
    unsigned int next_id = users[idx].last_msg_id + 1;
    if (next_id == 0) next_id = 1; /* evitar id=0 */

    msg->id = next_id;
    users[idx].last_msg_id = next_id;

    strncpy(msg->sender,   sender,   MAX_NAME     - 1); msg->sender[MAX_NAME - 1]     = '\0';
    strncpy(msg->text,     text,     MAX_MSG_TEXT - 1); msg->text[MAX_MSG_TEXT - 1]   = '\0';
    strncpy(msg->filename, filename ? filename : "",
            MAX_FILENAME - 1); msg->filename[MAX_FILENAME - 1] = '\0';
    msg->next = NULL;
...
static int deliver_message(const char *ip, int port, const char *sender,
    ...
    if (send_string(fd, OP_SEND_MESSAGE) < 0 ||
        send_string(fd, sender)          < 0 ||
        send_string(fd, id_str)          < 0 ||
        send_string(fd, text)            < 0) {
        close(fd);
        return -1;
    }
...
static int send_ack(const char *ip, int port, unsigned int msg_id) {
    ...
    if (send_string(fd, OP_SEND_MESS_ACK) < 0 ||
        send_string(fd, id_str)           < 0) {
        close(fd);
        return -1;
    }
 ...
 */
static int send_attach_ack(const char *ip, int port,
 ...
 */
static void handle_register(int client_fd, const char *client_ip __attribute__((unused))) {
    char name[MAX_NAME];

    if (recv_string(client_fd, name, sizeof(name)) < 0) return;
 ...
 */
static void handle_unregister(int client_fd, const char *client_ip __attribute__((unused))) {
    char name[MAX_NAME];
...
static void handle_connect(int client_fd, const char *client_ip) {
    ...
    if (recv_string(client_fd, name,     sizeof(name))     < 0) return;
    if (recv_string(client_fd, port_str, sizeof(port_str)) < 0) return;
    ...
    while (msg) {
        ...
        if (strlen(msg->filename) > 0) {
            /* Mensaje con adjunto */
            delivered = (deliver_message_attach(ip_copy, port_copy,
                            msg->sender, msg->id, msg->text, msg->filename) == 0);
        } else {
            /* Mensaje simple */
            delivered = (deliver_message(ip_copy, port_copy,
                            msg->sender, msg->id, msg->text) == 0);
        }
        ...
        if (delivered) {
            ...
            /* No se pudo entregar: volver a encolar */
            pthread_mutex_lock(&users_mutex);
            /* Re-insertar al principio para respetar el orden */
            msg->next = users[idx].pending;
            users[idx].pending = msg;
            pthread_mutex_unlock(&users_mutex);
            msg = next;
            continue;
 ...
 */
static void handle_disconnect(int client_fd, const char *client_ip __attribute__((unused))) {
    char name[MAX_NAME];
 ...
 * Almacena el mensaje, responde con el id y entrega si el destinatario esta conectado.
 */
static void handle_send(int client_fd, const char *client_ip __attribute__((unused))) {
    char sender[MAX_NAME], dest[MAX_NAME], text[MAX_MSG_TEXT];

    if (recv_string(client_fd, sender, sizeof(sender)) < 0) return;
    if (recv_string(client_fd, dest,   sizeof(dest))   < 0) return;
    if (recv_string(client_fd, text,   sizeof(text))   < 0) return;
        ...
        if (deliver_message(dest_ip, dest_port, sender, msg_id, text) == 0) {
            ...
            while (cur) {
                if (cur->id == msg_id) {
                    ...
                    else users[didx].pending = cur->next;
                    free(cur);
                    break;
                }
                prev = cur;
            ...
            /* Error de entrega: marcar destinatario como desconectado */
            pthread_mutex_lock(&users_mutex);
 ...
 */
static void handle_sendattach(int client_fd, const char *client_ip __attribute__((unused))) {
    char sender[MAX_NAME], dest[MAX_NAME], text[MAX_MSG_TEXT], filename[MAX_FILENAME];

    if (recv_string(client_fd, sender,   sizeof(sender))   < 0) return;
    if (recv_string(client_fd, dest,     sizeof(dest))     < 0) return;
    if (recv_string(client_fd, text,     sizeof(text))     < 0) return;
    if (recv_string(client_fd, filename, sizeof(filename)) < 0) return;
    ...
    int  dest_connected   = users[didx].connected;
    char dest_ip[MAX_IP]; int dest_port;
    strncpy(dest_ip, users[didx].ip, MAX_IP - 1);
    dest_ip[MAX_IP - 1] = '\0';
    dest_port = users[didx].port;

    int  sender_connected = users[sidx].connected;
    char sender_ip[MAX_IP]; int sender_port;
    strncpy(sender_ip, users[sidx].ip, MAX_IP - 1);
    sender_ip[MAX_IP - 1] = '\0';
    sender_port = users[sidx].port;
    ...
    if (dest_connected) {
        if (deliver_message_attach(dest_ip, dest_port,
                                   sender, msg_id, text, filename) == 0) {
            pthread_mutex_lock(&users_mutex);
 ...
 */
static void handle_users(int client_fd, const char *client_ip __attribute__((unused))) {
    char requester[MAX_NAME];

    if (recv_string(client_fd, requester, sizeof(requester)) < 0) return;
...
static void *client_thread(void *arg) {
    ...
    if      (strcmp(operation, OP_REGISTER)   == 0) handle_register(fd, ip);
    else if (strcmp(operation, OP_UNREGISTER) == 0) handle_unregister(fd, ip);
    else if (strcmp(operation, OP_CONNECT)    == 0) handle_connect(fd, ip);
    else if (strcmp(operation, OP_DISCONNECT) == 0) handle_disconnect(fd, ip);
    else if (strcmp(operation, OP_SEND)       == 0) handle_send(fd, ip);
    else if (strcmp(operation, OP_SENDATTACH) == 0) handle_sendattach(fd, ip);
    else if (strcmp(operation, OP_USERS)      == 0) handle_users(fd, ip);
    else {
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (client_fd < 0) {
            if (errno == EINTR) continue; /* senyal recibida, reintentar */
            perror("accept");
            break;
        }
